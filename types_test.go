...
func TestAck(t *testing.T) {
	...
	if a1 != a2 {
		t.Fatalf("expected %d; got %d", a1, a2)
	}
}
...
func TestDataMarshalBinary(t *testing.T) {
	...
	for { // each iteration simulates a read from the client
		...
		opcode := OperationCode(binary.BigEndian.Uint16(p[:2]))
		if opcode != OpData {
		...
		if blockcode != previous+1 {
			t.Fatalf("block number %d is not sequential; previous was = %d", blockcode, previous)
		}
